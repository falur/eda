...
const ORHESTRA_EXECUTION_CHOICES = [
  ...
  {
    value: 'single',
    name: 'Все этапы выполнять текущим агентом без передачи режима внутрь'
  },
  {
    value: 'single_propagated',
    name: 'Все этапы выполнять текущим агентом и передавать single совместимым скилам'
  }
];
...
function migrateSettingsContent(content) {
  ...
  transfer(
    'orhestra.execution',
    orhestra.execution,
    oneOf(['subagents', 'single', 'single_propagated']),
...
orhestra:
  ...
  # Где выполняются этапы и передаётся ли single совместимым вложенным скилам.
  # subagents | single | single_propagated
