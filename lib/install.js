...
// Среда считается установленной, только если в её папке скилов лежит хотя бы один eda-скил.
// Используется при массовом обходе (update-all), чтобы не засыпать скилами проекты,
// где папка .claude/skills или .codex/skills есть, но eda-скилов в ней нет.
async function detectInstalledTargets(cwd) {
  const targets = [];
  if (await hasInstalledSkill(path.join(cwd, '.claude/skills'))) targets.push('claude');
  if (await hasInstalledSkill(path.join(cwd, '.codex/skills'))) targets.push('codex');
  return targets;
}
