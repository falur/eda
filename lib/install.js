...
const FLOW_META_CLAUDE_MODELS = ['fable', 'haiku', 'sonnet', 'opus'];
const FLOW_META_CODEX_MODELS = ['gpt-5.6-luna', 'gpt-5.6-terra', 'gpt-5.6-sol'];
const FLOW_EXECUTE_MODES = ['inherit', 'single', 'subagents', 'auto'];
...
function migrateSettingsContent(content) {
  ...
  const flow = objectValue(raw.flow);
  transfer('flow.mode', flow.mode, oneOf(['automatic', 'manual']), value => { settings.flow.mode = value; });
  const flowMetaModel = objectValue(flow.meta_model);
  transfer(
    'flow.metaModel.claude',
    flowMetaModel.claude,
    oneOf(FLOW_META_CLAUDE_MODELS),
    value => { settings.flow.metaModel.claude = value; }
  );
  transfer(
    'flow.metaModel.codex',
    flowMetaModel.codex,
    oneOf(FLOW_META_CODEX_MODELS),
    value => { settings.flow.metaModel.codex = value; }
  );
  const flowPlanReview = objectValue(flow.plan_review);
  transfer('flow.planReview.enabled', flowPlanReview.enabled, bool, value => { settings.flow.planReview.enabled = value; });
  transfer('flow.planReview.maxCycles', flowPlanReview.max_cycles, positiveInt, value => { settings.flow.planReview.maxCycles = value; });
  const flowExecute = objectValue(flow.execute);
  transfer(
    'flow.execute.mode',
    flowExecute.mode,
    oneOf(FLOW_EXECUTE_MODES),
    value => { settings.flow.execute.mode = value; }
  );
  const flowManualTest = objectValue(flow.manual_test);
  transfer(
    'flow.manualTest.mode',
    flowManualTest.mode,
    oneOf(['auto', 'always', 'off']),
    value => { settings.flow.manualTest.mode = value; }
  );
  transfer('flow.manualTest.maxCycles', flowManualTest.max_cycles, positiveInt, value => { settings.flow.manualTest.maxCycles = value; });
  const flowCodeReview = objectValue(flow.code_review);
  transfer('flow.codeReview.enabled', flowCodeReview.enabled, bool, value => { settings.flow.codeReview.enabled = value; });
  transfer('flow.codeReview.maxCycles', flowCodeReview.max_cycles, positiveInt, value => { settings.flow.codeReview.maxCycles = value; });
...
function formatSettings(settings) {
  ...
  const flow = { ...DEFAULT_FLOW, ...(settings.flow ?? {}) };
  flow.metaModel = { ...DEFAULT_FLOW.metaModel, ...(flow.metaModel ?? {}) };
  flow.planReview = { ...DEFAULT_FLOW.planReview, ...(flow.planReview ?? {}) };
  flow.execute = { ...DEFAULT_FLOW.execute, ...(flow.execute ?? {}) };
  flow.manualTest = { ...DEFAULT_FLOW.manualTest, ...(flow.manualTest ?? {}) };
  flow.codeReview = { ...DEFAULT_FLOW.codeReview, ...(flow.codeReview ?? {}) };
...
flow:
  ...
  # automatic | manual
  mode: ${flow.mode ?? DEFAULT_FLOW.mode}
  meta_model:
    # Модель мета-ревьюера плана и ревьюера кода в eda-flow.
    # Модель для Claude Code: fable | haiku | sonnet | opus
    claude: ${flow.metaModel.claude ?? DEFAULT_FLOW.metaModel.claude}
    # Модель для Codex: gpt-5.6-luna | gpt-5.6-terra | gpt-5.6-sol
    codex: ${flow.metaModel.codex ?? DEFAULT_FLOW.metaModel.codex}
  plan_review:
    # Запускает проверки плана и мета-ревью в первой сессии.
    # true | false
    enabled: ${flow.planReview.enabled !== false ? 'true' : 'false'}
    # Сколько раз повторять «правка плана → мета-ревью».
    # Положительное целое число.
    max_cycles: ${flow.planReview.maxCycles ?? DEFAULT_FLOW.planReview.maxCycles}
  execute:
    ...
    mode: ${flow.execute.mode ?? DEFAULT_FLOW.execute.mode}
  manual_test:
    # Когда запускать eda-manual-test во второй сессии.
    # auto — только при визуальных изменениях, always — всегда, off — не запускать.
    mode: ${flow.manualTest.mode ?? DEFAULT_FLOW.manualTest.mode}
    # Сколько раз повторять «eda-fix → eda-manual-test».
    # Положительное целое число.
    max_cycles: ${flow.manualTest.maxCycles ?? DEFAULT_FLOW.manualTest.maxCycles}
  code_review:
    # Запускает третью сессию: eda-review и eda-fix-by-review.
    # true | false
    enabled: ${flow.codeReview.enabled !== false ? 'true' : 'false'}
    # Сколько раз повторять «eda-review → eda-fix-by-review».
    # Положительное целое число.
    max_cycles: ${flow.codeReview.maxCycles ?? DEFAULT_FLOW.codeReview.maxCycles}
