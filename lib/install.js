...
const PLAN_REVIEW_AGENT_DEFINITIONS = [
  {
    key: 'requirements',
    description: 'Проверяет соответствие плана задаче, scope, критериям готовности и применимым business-требованиям.',
    auto: 'Эта базовая проверка по умолчанию запускается всегда.',
    mode: 'always',
    claude: 'sonnet',
    codex: 'gpt-5.6-terra'
  ...
  {
    key: 'architecture',
    description: 'Проверяет архитектурные границы, зависимости и соответствие docs/arch.md.',
    auto: 'Эта базовая проверка по умолчанию запускается всегда.',
    mode: 'always',
    claude: 'opus',
    codex: 'gpt-5.6-sol'
  ...
  {
    key: 'feasibility',
    description: 'Проверяет реализуемость плана относительно фактического кода и отсутствие скрытого исследования.',
    auto: 'Эта базовая проверка по умолчанию запускается всегда.',
    mode: 'always',
    claude: 'opus',
    codex: 'gpt-5.6-sol'
  ...
  {
    key: 'execution',
    description: 'Проверяет связность фаз, ID задач, зависимости и размер работы для одного исполнителя.',
    auto: 'Эта базовая проверка по умолчанию запускается всегда.',
    mode: 'always',
    claude: 'sonnet',
    codex: 'gpt-5.6-terra'
  ...
  {
    key: 'verification',
    description: 'Проверяет тесты, команды проверок, ошибки, логирование, документацию и эксплуатационную готовность.',
    auto: 'Эта базовая проверка по умолчанию запускается всегда.',
    mode: 'always',
    claude: 'sonnet',
    codex: 'gpt-5.6-terra'
  ...
  {
    key: 'api',
    description: 'Проверяет API, DTO, события, очереди, внешние интеграции и совместимость контрактов.',
    auto: 'В auto запускается при изменении API, событий, очередей или внешних интеграций.',
    mode: 'auto',
    claude: 'sonnet',
    codex: 'gpt-5.6-terra'
  },
  {
    key: 'database',
    description: 'Проверяет схему данных, миграции, backfill, индексы, транзакции и rollback.',
    auto: 'В auto запускается при изменении хранения, моделей, SQL, схем или миграций.',
    mode: 'auto',
    claude: 'opus',
    codex: 'gpt-5.6-sol'
  },
  {
    key: 'security',
    description: 'Проверяет auth, permissions, внешний ввод, секреты, персональные данные и опасные операции.',
    auto: 'В auto запускается для auth, permissions, внешнего ввода, секретов и чувствительных данных.',
    mode: 'auto',
    claude: 'opus',
    codex: 'gpt-5.6-sol'
  },
  {
    key: 'frontend',
    description: 'Проверяет пользовательский сценарий, состояния UI, ошибки, accessibility и адаптивность.',
    auto: 'В auto запускается для компонентов, стилей, клиентских маршрутов и состояния UI.',
    mode: 'auto',
    claude: 'sonnet',
    codex: 'gpt-5.6-terra'
  },
  {
    key: 'performance',
    description: 'Проверяет нагрузку, запросы, конкурентность, кеши и горячие пути.',
    auto: 'В auto запускается для запросов, пакетной обработки, кешей, конкурентности и горячих путей.',
    mode: 'auto',
    claude: 'opus',
    codex: 'gpt-5.6-sol'
  },
  {
    key: 'previous',
    description: 'Проверяет закрытие findings предыдущей связанной итерации plan-review.',
    auto: 'В auto запускается только при наличии связанной цепочки review/fix для текущего SHA-256 плана.',
    mode: 'auto',
    claude: 'haiku',
    codex: 'gpt-5.6-luna'
  }
];
const DEFAULT_PLAN_REVIEW_AGENTS = Object.fromEntries(PLAN_REVIEW_AGENT_DEFINITIONS.map(agent => [
  agent.key,
  {
    mode: agent.mode,
    model: {
...
const DEFAULT_SETTINGS = {
  ...
  planReview: {
    threshold: 95,
    agents: DEFAULT_PLAN_REVIEW_AGENTS
  },
  planPolish: {
    limit: 3
  },
...
const PLAN_REVIEW_THRESHOLD_CHOICES = [90, 95, 100].map(value => ({
  value,
  name: `${value}/100`
}));
const PLAN_POLISH_LIMIT_CHOICES = [1, 2, 3, 5, 10].map(value => ({
  value,
  name: `${value} ${value === 1 ? 'итерация' : value < 5 ? 'итерации' : 'итераций'}`
}));
  ...
  for (const projectDir of projects) {
    ...
    try {
      ...
      if (resolvedSettingsMode === 'skip' && await fileExists(projectSettingsPath)) {
        ...
        if (migrated.version === SETTINGS_VERSION && migrated.missingKeys.size === 0) {
          ...
          output.write(`Старый или неполный файл настроек перенесён на version: ${SETTINGS_VERSION}: ${SETTINGS_RELATIVE_PATH}\n`);
        }
      } else {
...
export async function askSettings({
  ...
  if (requestedSections.has('plan-review')) {
    settings.planReview.agents = await askPlanReviewAgentSettings({
      input,
      output,
      selectPrompt,
      seedAgents: settings.planReview.agents,
      missingKeys
    });
  }
...
export async function askPlanReviewAgentSettings({
  input = process.stdin,
  output = process.stdout,
  selectPrompt = select,
  seedAgents = DEFAULT_PLAN_REVIEW_AGENTS,
  missingKeys = null
} = {}) {
  return askAgentSettings({
    input,
    output,
    selectPrompt,
    definitions: PLAN_REVIEW_AGENT_DEFINITIONS,
    seedAgents,
    missingKeys,
    keyPrefix: 'planReview.agents',
    label: 'plan-review'
  });
}

async function askAgentSettings({ input, output, selectPrompt, definitions, seedAgents, missingKeys, keyPrefix, label }) {
  ...
  for (const agent of definitions) {
    ...
    if (mode !== 'off' && shouldAsk(claudeKey)) {
    ...
    if (mode !== 'off' && shouldAsk(codexKey)) {
...
function migrateSettingsContent(content) {
  ...
  transferAgentSettings({
    rawAgents: objectValue(planReview.agents),
    settingsAgents: settings.planReview.agents,
    definitions: PLAN_REVIEW_AGENT_DEFINITIONS,
    prefix: 'planReview.agents',
    missingKeys
  });
...
${onFailureThen}
  ...
  const planReviewAgents = formatAgents(
    PLAN_REVIEW_AGENT_DEFINITIONS,
    settings.planReview.agents,
    DEFAULT_PLAN_REVIEW_AGENTS
  );
...
plan:
  ...
  # Запускает один eda-plan-review и при необходимости один eda-plan-review-fix.
  # true | false
...
plan-review:
  # План готов только без обязательных findings и при score не ниже этого порога.
  # 1..100
