...
const SETTINGS_CHOICES = [
  ...
  {
    section: 'plan',
    value: 'planMetaReview',
    name: 'Проверять eda-plan тремя субагентами',
    checked: DEFAULT_SETTINGS.plan.metaReview
  },
  {
    section: 'plan-polish',
...
const UPDATE_ALL_SETTINGS_MODE_CHOICES = [
  ...
  {
    value: 'skip',
    name: 'Skip — сохранить существующие конфиги, defaults создать только там, где файла нет'
  }
];
...
export async function updateAll({
  ...
  if (resolvedSettingsMode === 'configure') {
    ...
    output.write(`Режим skip: существующие ${SETTINGS_RELATIVE_PATH} сохраняются, defaults создаются только для проектов без файла.\n`);
  ...
  for (const projectDir of projects) {
    ...
    try {
      ...
      if (resolvedSettingsMode === 'skip' && await fileExists(projectSettingsPath)) {
        output.write(`Существующий файл настроек сохранён: ${SETTINGS_RELATIVE_PATH}\n`);
      } else {
...
export async function askUpdateAllSettingsMode({
  ...
  if (!input.isTTY || !output.isTTY) {
    output.write(`Нет интерактивного терминала — использую режим skip: сохраняю существующие ${SETTINGS_RELATIVE_PATH}, defaults создаю только при отсутствии файла.\n`);
    return 'skip';
  }
...
export async function askSettings({
  ...
  if (!input.isTTY || !output.isTTY) {
    output.write(`Нет интерактивного терминала — использую настройки ${SETTINGS_RELATIVE_PATH} по умолчанию.\n`);
    return structuredClone(DEFAULT_SETTINGS);
  }

  const requestedSections = new Set(sections);
  const booleanChoices = SETTINGS_CHOICES.filter(choice => requestedSections.has(choice.section));
  const selected = booleanChoices.length > 0
    ? await checkboxPrompt({
        message: 'Какие настройки включить?',
        instructions: 'Стрелки — выбрать, Space — отметить, Enter — продолжить',
        choices: booleanChoices.map(({ section, ...choice }) => choice)
      }, {
        input,
        output
      })
  ...
  const orhestraMode = requestedSections.has('orhestra')
    ? await selectPrompt({
    ...
    : DEFAULT_SETTINGS.orhestra.mode;

  const enabledOrhestraSteps = requestedSections.has('orhestra')
    ? await checkboxPrompt({
    ...
    : DEFAULT_ORHESTRA_STEPS.filter(step => step.enabled).map(step => step.skill);

  const polishLimit = requestedSections.has('orhestra') && enabledOrhestraSteps.includes('eda-polish')
    ? await selectPrompt({
  ...
  const aimMode = requestedSections.has('aim')
    ? await selectPrompt({
    ...
    : DEFAULT_SETTINGS.aim.mode;

  const exploreDecisionMode = requestedSections.has('explore')
    ? await selectPrompt({
  ...
  const planSize = requestedSections.has('plan')
    ? await selectPrompt({
  ...
  const planDecisionMode = requestedSections.has('plan')
    ? await selectPrompt({
    ...
    : DEFAULT_SETTINGS.plan.decisionMode;

  const testStrategy = requestedSections.has('plan')
    ? await selectPrompt({
  ...
  const loggingStrategy = requestedSections.has('plan')
    ? await selectPrompt({
  ...
  const reviewAgents = requestedSections.has('review')
    ? await askReviewAgentSettings({ input, output, selectPrompt })
    : structuredClone(DEFAULT_REVIEW_AGENTS);
  ...
  return {
    ...
    plan: {
      ...
      metaReview: requestedSections.has('plan')
        ? selected.includes('planMetaReview')
        : DEFAULT_SETTINGS.plan.metaReview,
      size: planSize,
...
async function configureSettings(cwd, { input = process.stdin, output = process.stdout } = {}) {
  ...
  if (existingContent !== null && readTopLevelYamlScalar(existingContent, 'version') === '2') {
    const presentSections = readTopLevelYamlKeys(existingContent);
    const missingSections = SETTINGS_SECTION_NAMES.filter(section => !presentSections.has(section));
    if (missingSections.length === 0) {
      output.write(`Настройки ${SETTINGS_RELATIVE_PATH} версии 2 полные — вопросы не требуются.\n`);
      return;
    }

    output.write(`Дополняю отсутствующие разделы ${SETTINGS_RELATIVE_PATH}: ${missingSections.join(', ')}.\n`);
    const settings = await askSettings({ input, output, sections: missingSections });
    const missingContent = extractSettingsSections(formatSettings(settings), missingSections);
    const separator = existingContent.endsWith('\n\n') ? '' : existingContent.endsWith('\n') ? '\n' : '\n\n';
    const updatedContent = `${existingContent}${separator}${missingContent}\n`;
    await writeSettingsContent(cwd, updatedContent);
    output.write(`Добавлены разделы ${SETTINGS_RELATIVE_PATH}: ${missingSections.join(', ')}.\n`);
    return;
  }
...
function migrateSettingsV1(content) {
  ...
  return {
    ...
    plan: {
      strict: defaultsStrict,
      metaReview: DEFAULT_SETTINGS.plan.metaReview,
      size: readLegacyEnum(
        content,
        'defaults',
        'plan_size',
...
function readTopLevelYamlKeys(content) {
  const keys = new Set();
  for (const line of content.replace(/^\uFEFF/, '').split(/\r?\n/)) {
...
function extractSettingsSections(content, sectionNames) {
  const requested = new Set(sectionNames);
  const lines = content.split(/\r?\n/);
  const sectionStarts = [];
  for (let index = 0; index < lines.length; index += 1) {
    const match = lines[index].match(/^([A-Za-z0-9_-]+):(?:\s.*)?$/);
    if (match) sectionStarts.push({ name: match[1], index });
  }
  ...
  for (let index = 0; index < sectionStarts.length; index += 1) {
    ...
    blocks.push(lines.slice(section.index, end).join('\n').trimEnd());
  }

  const found = new Set(blocks.map(block => block.match(/^([A-Za-z0-9_-]+):/)?.[1]));
  const missing = sectionNames.filter(section => !found.has(section));
  if (missing.length > 0) {
    throw new Error(`Не удалось сформировать разделы ${SETTINGS_RELATIVE_PATH}: ${missing.join(', ')}.`);
  }
  return blocks.join('\n\n');
}
...
plan:
  ...
  # Включает мета-ревью плана тремя субагентами.
  # true | false
  meta_review: ${settings.plan.metaReview !== false ? 'true' : 'false'}
  # Задаёт размер плана.
  # normal | short | ask_each_time
