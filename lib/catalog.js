...
function validateSkillConfig(skillName, config) {
  ...
  if (config.claude && (typeof config.claude !== 'object' || Array.isArray(config.claude))) {
    throw new Error(`Скил ${skillName}: claude должен быть объектом.`);
  }
  if (config.claude?.context && config.claude.context !== 'fork') {
    throw new Error(`Скил ${skillName}: claude.context поддерживает только fork.`);
  }
  if (config.claude?.agent && (typeof config.claude.agent !== 'string' || !config.claude.agent.trim())) {
    throw new Error(`Скил ${skillName}: claude.agent должен быть непустой строкой.`);
  }
  if (config.claude?.agent && config.claude.context !== 'fork') {
    throw new Error(`Скил ${skillName}: claude.agent требует claude.context fork.`);
  }
}
