...
- у каждого finding есть fingerprint, severity, plan_location, problem, evidence, impact, fix, acceptance;
...
2. Прими finding только когда план обязательно нужно изменить: есть точное место, доказательство, практическая ошибка или существенная неоднозначность, минимальная правка и проверяемая приёмка. Если замечание можно без последствий не применять, это не finding.
...
5. Объедини findings с одной первопричиной и одной правкой, сохранив все checks и evidence.
6. Для совпадающего fingerprint сохрани прежний ID. Новому finding дай следующий свободный `PR-NNN`; номер не переиспользуй.
7. В итог включай только открытые findings и состояния прошлых ID. Предложения, которые не требуют обязательной правки плана, сохраняй только в `Отклонённые предложения` и не учитывай в score.
...
- `100`: открытых findings нет, план конкретен и исполним.
- `95–99`: остаются только локальные, но обязательные low findings.
- `80–94`: есть medium или несколько low findings.
- `60–79`: есть high findings.
...
Статус `ready` ставь только когда нет открытых findings и `$SCORE >= $THRESHOLD`. Любой открытый finding даёт `changes-required`, независимо от score. Score ниже threshold без открытых findings недопустим: не снижай оценку за необязательные пожелания и не выдумывай finding только ради числа.
...
- Усреднять score агентов, менять score без изменения findings или снижать его за необязательные пожелания.
- Принимать косметическое расширение текста, дополнительное объяснение или вкусовую формулировку как finding.
