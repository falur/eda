...
В normal/strict запусти `eda-review-check` для `$REVIEW_FILE`. Если пользователь явно написал `strict` или `normal`, передай этот режим: прямое указание действует на всю запрошенную цепочку. Если режим получен только из `review.strict`, не передавай его: вызванный `eda-review-check` должен отдельно прочитать `review-check.strict` и `review-check.include_code_quality`. Именно `eda-review-check` запускает `architecture-check`, `rules-check`, а если `plan` в front matter указывает на файл — ещё и `plan-check`; при включённой настройке `quality-check` тоже запускается. Затем `eda-review-check` обновляет `status`, `meta_reviewers`, `score` и раздел `## Изменения после мета-ревью`.
