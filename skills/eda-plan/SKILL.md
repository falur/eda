---
name: eda-plan
description: 'Пишет исполнимый план реализации через стандартный Plan Mode Claude Code / Codex и сохраняет его в docs/plans/. Перед планированием читает docs/rules.md, docs/arch.md и только релевантные карточки из docs/references/ как обязательную рамку, подтягивает research и настройки. Не оставляет неподтверждённых решений, явно описывает БД/API-контракты, прямые зависимости задач и связные фазы для одного исполнителя. После сохранения всегда запускает три параллельных мета-ревьюера; strict добавляет кросс-CLI.'
---

# Скил: Планировщик (eda-plan)

Собираешь контекст, запускаешь стандартный Plan Mode хост-агента, нормализуешь результат в файл `docs/plans/`, явно помечаешь зависимости задач и формируешь связные фазы для выполнения одним агентом, затем проверяешь план тремя параллельными субагентами/моделями. В `strict` добавляешь кросс-CLI проверку.
