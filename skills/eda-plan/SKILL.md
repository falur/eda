...
Собираешь контекст, запускаешь стандартный Plan Mode хост-агента, нормализуешь результат в файл `docs/plans/`, явно помечаешь зависимости задач и формируешь связные фазы для выполнения одним агентом. По настройкам добавляешь мета-ревью тремя параллельными субагентами/моделями и независимую кросс-CLI проверку.

## Режим запуска

Прочитай `docs/settings.yaml`, если файл есть. Прямое указание пользователя в текущем сообщении важнее настроек.

Поддерживается только `version: 2`. При другой версии, отсутствующем разделе или неизвестном значении используй дефолт; настройки старого формата не применяй. Дефолты: `plan.strict: false`, `plan.meta_review: true`, `plan.size: normal`, `plan.decision_mode: recommend_and_ask`, `plan.test_strategy: ask_each_time`, `plan.logging_strategy: ask_each_time`.

| Настройка | Значения | Аргументы в сообщении |
|---|---|---|
| `plan.strict` | `true`, `false` | `strict`; выключение: `normal`, `без strict`, `без строгого режима` |
| `plan.meta_review` | `true`, `false` | включение: `с мета-ревью`, `с проверкой субагентами`; выключение: `без мета-ревью`, `без проверки субагентами` |
...
`без проверок` одновременно устанавливает `plan.meta_review: false` и `plan.strict: false`. `plan.meta_review` и `plan.strict` независимы: можно оставить только один вид проверки. Более конкретное прямое указание в том же сообщении имеет приоритет.
...
Если `plan.meta_review: false`, пропусти этот этап, оставь `meta_reviewers: []` и не добавляй `Изменения после мета-ревью`. Если открытых вопросов нет, перейди к strict-этапу или финалу. Не запускай субагентов и CLI-fallback.
...
Промпт всем: прочитать план, `docs/rules.md`, `docs/arch.md`, применимые карточки из `sources.business` и `sources.references`, research из `sources.research` при наличии и релевантный код по глубине модели. Найти пропущенные шаги, нарушения бизнес-требований, правил, архитектуры и эталонов, необоснованные отклонения, скрытые риски, риски для смежного кода, недостающие тесты, шаги без критериев готовности, неверные зависимости и слишком крупные или несвязные фазы для одного исполнителя. Формат ответа: `+ N | − N | ~ N: причина`; флагману явно скажи открывать файлы модулей, API, тестов и сценариев, на которые ссылается план.
...
`plan.strict` не зависит от `plan.meta_review`. Обычный режим пропускает этот этап. В `strict` отдай файл соседнему CLI:
...
Коротко сообщи режим, какие из двух проверок выполнялись или были отключены, путь к плану, путь к review-файлу в `strict`, 3–5 главных шагов простыми словами и что дальше делает `eda-plan-execute`.
...
- Пропускать включённое мета-ревью тремя моделями или запускать ревьюеров частями.
