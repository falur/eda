...
Агенты устанавливаются отдельно от скилов. `eda-commit` только сохраняет исходный запрос, запускает их по очереди и передаёт полный структурированный результат первого второму.

Для Claude Code установленный `eda-commit` получает `model: haiku`. В Codex у скилов пока нет отдельного per-skill model override: основной поток остаётся на выбранной пользователем модели, но он выполняет только лёгкую оркестрацию, а содержательная работа закреплена за `gpt-5.6-luna` и `gpt-5.6-terra` в agent-файлах.
...
Исходники скилов лежат в `skills/<skill>/SKILL.md`. Дополнительные ресурсы скила можно хранить рядом; `skill.json` содержит только настройки рендеринга и в проект пользователя не копируется.
