...
| `eda-plan-polish` | Полирует уже готовый план до оценки 95/100 или заданного порога. Каждая итерация сразу запускает три изолированных параллельных агента на быстрой, кодовой и флагманской моделях. В strict дополнительно отдаёт план соседнему CLI (Claude ↔ Codex) на кросс-ревью. Основной агент после ответов выставляет оценку, анализирует предложения, содержательно правит только файл плана при оценке ниже порога и не запускает проверки проекта | `docs/plans/` |
...
| `eda-review` | Делает код-ревью с оценкой 0–100 и сверкой с планом работ, но в файл пишет только проблемы: ошибки, недоделки, неточности, нарушения правил/архитектуры, риски и недостающие тесты. Сверяет с планом, только если он указан или однозначно найден; иначе ревьюит без сверки с планом и про план не спрашивает. Без аргументов ревьюит незакоммиченный `git diff HEAD`. Не перечисляет выполненную работу. В обычном режиме после черновика передаёт файл в `eda-review-check`; с флагом `draft` сохраняет только первичное ревью без субагентов | `docs/reviews/` |
...
| `eda-fix-by-review` | Применяет замечания из ревью. Обязательные — все сразу. Optional-пункты в обычном режиме спрашивает, а в `apply-optional` сам решает: применяет полезные, отклонённые обосновывает в отчёте. В конце ссылается на отчёт прямо в файле ревью | `docs/review-fixes/` |
...
| `eda-docs` | Создаёт или обновляет лаконичные правила проекта, текущую и предлагаемую архитектуру со структурой папок, компактный `AGENTS.md` со ссылками на обязательные документы и минимальный `CLAUDE.md`, который требует читать и соблюдать `AGENTS.md`. Без явного выбора обновляет весь комплект основных документов | `docs/rules.md`, `docs/arch.md`, `AGENTS.md`, `CLAUDE.md` |
