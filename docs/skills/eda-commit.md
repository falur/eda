---
name: eda-commit
description: 'Формирует коммит из незакоммиченных изменений в стиле проекта (по docs/rules.md или истории), коммитит без `--no-verify`. В конце спрашивает интерактивно: push (если есть remote) / merge в main + переключение + удаление ветки (если не в main) / ничего. Не правит логику кода, не пишет тесты — это `eda-execute` или `eda-fix-by-review`.'
---
