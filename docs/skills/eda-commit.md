...
5. **Прочитай `docs/rules.md` и `docs/arch.md`**, если есть, и строго следуй им при выборе файлов, сообщении коммита и дальнейших действиях.
6. **Не пропускай `docs/**`.** Планы, исследования, журналы, ревью, отчёты о фиксах, автоматизации, `docs/rules.md` и `docs/arch.md`, созданные или обновлённые скилами, — часть изменения. Если они относятся к текущей задаче, добавляй их в коммит вместе с кодом.
7. **Простой язык** в сообщениях пользователю.
...
`git status`, `git diff --stat`, `git log -5 --oneline`, `docs/rules.md`, `docs/arch.md` (если есть) — определи стиль коммитов проекта и строго следуй правилам/архитектуре. Смотри и tracked, и untracked файлы.
