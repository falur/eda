---
name: eda-execute
description: 'Исполняет план из docs/plans/ — перед правками читает `docs/rules.md` и `docs/arch.md` и строго следует им, правит код, пишет тесты к изменениям внутри каждого шага, в конце прогоняет полный сьют тестов и линтеры. Прогресс — чекбоксы в файле плана + журнал в `docs/executions/`; в конец плана дописывается ссылка на журнал. Не коммитит (это `eda-commit`), не делает ревью (это `eda-review`). Все вопросы — через интерактивный вопрос.'
---
