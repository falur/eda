...
| Среда | Слабая | Средняя | Мощная |
|---|---|---|---|
| Claude Code (`Agent` tool) | `model: "haiku"` | `model: "sonnet"` | `model: "opus"` |
| Codex CLI (`codex exec --model …`) | `gpt-5-nano` | `gpt-5-mini` | `gpt-5` |

В Codex запускай три параллельных `codex exec --model <id>` через `Bash` с `&`. Если какой-то из идентификаторов модели в твоей версии Codex недоступен — возьми ближайший аналог того же тира из `codex --help` или из доступных моделей.
