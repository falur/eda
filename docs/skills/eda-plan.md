...
Запусти **в одном сообщении** трёх агентов на трёх разных моделях — слабая, средняя, мощная (в Claude Code это `model: "haiku"`, `model: "sonnet"`, `model: "opus"`; в Codex — три параллельных `codex exec --model <id>` через `Bash` с `&` на трёх моделях разных уровней).
