...
В Codex — три параллельных `codex exec --model <id>` через `Bash` с `&`. Если какой-то идентификатор недоступен в твоей версии Codex — возьми ближайший аналог того же тира из `codex --help`.
