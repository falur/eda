...
Запусти **в одном сообщении** трёх агентов на разных моделях — Sonnet, Haiku, Opus. Каждому дай путь к файлу ревью + указание на `$TARGET` и попроси: «прочитай ревью и diff; предложи, какие пункты добавить (которых не хватает), какие убрать (избыточные/неверные), какие переформулировать. Список `+ N | − N | ~ N: причина`. Без воды.»

В Codex CLI — три параллельных `codex exec --model <id>` через `Bash` с `&`.
