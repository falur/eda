...
Версия 1.0 переводит `eda` с набора файлов-скилов на пакет с каноническими скилами и специализированными агентами для Claude Code и Codex CLI. Установщик обновляет управляемые компоненты автоматически, но существующий `docs/settings.yaml` версии 1 нужно перенести вручную.
...
`update-all` ищет проекты на глубине до двух уровней, обновляет только уже найденные среды Claude/Codex и не создаёт и не меняет `docs/settings.yaml`.
...
Версия 1.0 поддерживает только формат настроек `version: 2`. Установщик намеренно сохраняет существующий файл без изменений, поэтому файл с `version: 1` нужно отредактировать вручную.
...
Минимальный перенос старых общих настроек выглядит так:
...
Новые разделы `orhestra`, `aim`, `review.agents` и `send-review` можно взять из полного актуального примера в [README](README.md). Если раздел или значение отсутствует, соответствующий скил использует документированный default.
