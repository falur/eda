...
Версия 3.0 перенесла все рабочие результаты скилов в `docs/artifacts/`; постоянная документация проекта остаётся непосредственно в `docs/`. Версия 3.1 дополнительно переработала проверку плана, ревью кода и циклы полировки. В npm 3.0 не публиковалась, поэтому переход выполняется сразу с 2.x на 3.1.
...
Версия CLI должна начинаться с `3.1`. После обновления перезапусти Claude Code или Codex CLI, чтобы среда перечитала новые скилы и агентов.
...
Версия npm-пакета и версия схемы настроек независимы: в eda 3.1 актуальным остаётся `docs/settings.yaml` с `version: 3`.
...
Что в 3.1 изменится в уже настроенном проекте:
...
- **Секция `plan-review.agents` больше не читается** и при нормализации настроек отбрасывается. Дефолт `plan-review.threshold` — `100`, `plan-polish.limit` — `2`.
- **У `eda-polish` больше нет порога.** Цикл завершается только при нуле находок и останавливается досрочно, если за круг число открытых находок не сократилось. Существующий валидный `docs/settings.yaml` установщик не переписывает, поэтому аргумент вида `args: "threshold 90 limit 2"` в шаге оркестратора останется в файле и будет игнорироваться — убери из него `threshold N` вручную.
...
- `eda --version` выводит версию `3.1.x`;
