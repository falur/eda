...
- **У `eda-review` появился режим исполнения.** `review.execution` принимает `subagents` или `single`; default `subagents` сохраняет прежнее поведение. В `single` все выбранные проверки выполняет один review-runner, поэтому модели отдельных `review.agents` не применяются.
- **У `eda-orhestra` появился режим исполнения.** `orhestra.execution` принимает `subagents`, `single` или `single_propagated`; default `subagents` сохраняет прежнее поведение. `single` выполняет верхние этапы текущим агентом, а `single_propagated` дополнительно передаёт `single` совместимым execute/review-этапам.
